// const express = require('express')
// const User = require('../models/user')
// const router = new express.Router()

// router.post('/users', async (req, res) => {
//     const user = new User(req.body)

//     try {
//         await user.save()
//         // res.status(201).send(user)
//         res.redirect('/profile')
//     } catch (e) {
//         res.status(400).send(e)
//     }
// })

// router.get('/users', async (req, res) => {
//     try {
//         const users = await User.find({})
//         res.send(users)
//     } catch (error) {
//         res.status(500).send
//     }
// })

// router.get('/users/:id', async (req, res) => {
//     const _id = req.params.id
//     try {
//         const user = await User.findById(_id)
//         if (!user) {
//             return res.status(404).send()
//         }
//         res.send(user)
//     } catch (e) {
//         res.status(500).send(e)
//     }
// })

// router.patch('/users/:id', async (req, res) => {

//     const updates = Object.keys(req.body)
//     const allowerdUpdates = ['name', 'email', 'password', 'age']
//     const isValidOperation = updates.every((update) => allowerdUpdates.includes(update))

//     if (!isValidOperation) {
//         return res.status(400).send({ error: 'Invalid updates!' })
//     }

//     try {

//         // bypassed more advanced features like middleware which means that if we want to use them consistently
//         // So "User.findByIdAndUpdate" will not work 
//         // updates.forEach((update) => req.user[update] = req.body[update])// Dynamic update  
//         // await req.user.save()

//         /*Before using "mongoose.Schema" in user.js*/
//         const user = await User.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true })

//         if(!user){
//             return res.status(404).send()
//         }
//         res.send(req.user)
//     } catch (e) {
//         res.status(400).send(e)
//     }
// })

// router.delete('/users/:id', async (req, res) => {
//     try {
//         const user = await User.findByIdAndDelete(req.user._id) // The id of the logedin user to delete him self

//         if (!user) {
//             return res.status(404).send()
//         }

//         // await req.user.remove()
//         // sendCancelEmail(req.user.email, req.user.name)
//         res.send(req.user)
//     } catch (e) {
//         res.status(500).send(e)
//     }
// })
// module.exports = router